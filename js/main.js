...
// Safety net: force content visible after 5s no matter what
setTimeout(() => {
  const app = document.getElementById('app-content');
  if (app) {
    app.style.visibility = 'visible';
...
// Boot on load (ensures GSAP deferred scripts are ready)
window.addEventListener('load', orchestrator);

// Emergency fallback: force page visible after 5 seconds no matter what
setTimeout(() => {
  const app = document.getElementById('app-content');
